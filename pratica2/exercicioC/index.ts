...
const aplicarFiltroMediana = (imagem: PNG, tamanhoFiltro: number[]) => {
  const matriz = gerarMatriz(imagem);
  const filtro: number[][] = Array(tamanhoFiltro[0]).fill(
    Array(tamanhoFiltro[1]).fill(0)
  );
  ...
  percorrerMatriz(matriz, (valor, linha, coluna) => {
    ...
    percorrerMatriz(filtro, (valorFiltro, linhaFiltro, colunaFiltro) => {
      ...
      if (
        matriz[linhaMatriz] === undefined ||
        matriz[linhaMatriz][colunaMatriz] === undefined
      )
    ...
    const mediana =
      valoresFiltroOrdenados[Math.floor(valoresFiltroOrdenados.length / 2)];
    matriz[linha][coluna] = mediana;
  });
  inserirMatriz(matriz, imagem);
};

const exercicioC = async (caminho: string, tamanho: number) => {
  const imagem = await carregar(caminho);
  aplicarFiltroMediana(imagem, [tamanho, tamanho]);
  await salvar(imagem);
};
